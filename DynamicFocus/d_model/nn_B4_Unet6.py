...
class ConvDown2D(nn.Module):
    """2D convolutional downsampling block: Convolution + BatchNorm + ReLU"""
...
class ConvUp2D(nn.Module):
    """2D convolutional upsampling block: Transposed Convolution + BatchNorm + ReLU"""
...
class UNet6(nn.Module):
    def __init__(self, in_channels, out_channels, base_channels=32, layer_num=4):
        ...
        for i in range(layer_num):
            self.enc_convs.append(ConvDown2D(current_in_channels, current_out_channels))
        ...
        self.bottleneck_conv = ConvDown2D(current_in_channels, current_out_channels)
        ...
        for i in range(layer_num):
            ...
            self.up_convs.append(ConvUp2D(decoder_in_channels, decoder_out_channels))
            ...
            self.dec_convs.append(ConvDown2D(dec_conv_in_channels, decoder_out_channels))
...
if __name__ == '__main__':
    ...
    for Cbase in [8, 16, 32, 64]:
        for Nlayer in [3, 4, 5]:
            model = UNet6(in_channels=in_channels, out_channels=out_channels, base_channels=Cbase, layer_num=Nlayer)  # Example for RGB input and 2-class output
